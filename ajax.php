...
try {
	...
	switch ($event['operation']) {
		case 'create':
			...
			$event_id = $capi->CreateCalendarEvent($event['calendarId'], $event['title'], $event['all_day'],  $event['recurrence'], $event['recurrence_end'], $event['event_time'], $_SESSION['user_timezone'], $_SESSION['access_token']);
		...
		case 'GetCalendarsList':
			// Delete event on primary calendar
			$GetCalendarsList = $capi->GetCalendarsList($_SESSION['access_token']);

			echo json_encode(['GetCalendarsList' => $GetCalendarsList]);
			break;
	}
} catch (Exception $e) {
